...
function invitationStatus(invitation: StaffInvitation) {
  ...
  if (new Date(invitation.expiresAt).getTime() <= Date.now()) {
    return { label: "Expired", color: "#991B1B", background: "#FEE2E2" };
  }
...
export function StaffInvitationsPage() {
  ...
  const { data: invitations = [], isLoading: invitationsLoading } = useGetStaffInvitationsQuery(
    organizationId,
    { skip: !organizationId },
  );
  ...
  const emailError = touched.email && !email.trim()
    ...
    : touched.email && !EMAIL_RE.test(email.trim())
      ...
      : "";
  const organizationError = touched.organization && !organizationId
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setApiError(apiMessage(error, "Could not create the invitation. Please try again."));
    }
  };
            ...
            <form onSubmit={submit} noValidate style={{ display: "flex", flexDirection: "column", gap: "var(--space-md)" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                ...
                <select
                  ...
                  style={{ padding: "8px 12px", border: organizationError ? "1px solid var(--color-critical)" : "1px solid var(--color-hairline-strong)", borderRadius: "var(--radius-sm)", background: "var(--color-surface)", color: "var(--color-ink)", font: "inherit" }}
                >
              ...
              <button type="submit" disabled={createState.isLoading || organizationsLoading} style={{ border: 0, borderRadius: "var(--radius-sm)", padding: "10px 14px", background: "var(--color-primary)", color: "var(--color-on-primary)", font: "inherit", fontWeight: 700, cursor: createState.isLoading ? "wait" : "pointer" }}>
                {createState.isLoading ? "Creating…" : "Create invitation"}
              </button>
            </form>
          </section>

          {createdInvitation && (
            <section role="status" style={{ background: "var(--color-surface)", border: "1px solid var(--color-primary)", borderRadius: "var(--radius-md)", padding: "var(--space-lg)", marginBottom: "var(--space-lg)" }}>
              ...
              <div style={{ display: "flex", gap: 8, alignItems: "stretch" }}>
                <code style={{ flex: 1, overflowWrap: "anywhere", padding: "10px 12px", background: "var(--color-surface-sunken)", borderRadius: "var(--radius-sm)", color: "var(--color-ink)" }}>{createdInvitation.token}</code>
                <button type="button" onClick={copyToken} style={{ border: "1px solid var(--color-hairline-strong)", borderRadius: "var(--radius-sm)", background: "var(--color-surface)", color: "var(--color-ink)", padding: "0 12px", cursor: "pointer", font: "inherit" }}>{copied ? "Copied" : "Copy"}</button>
              </div>
            </section>
          )}

          <section style={{ gridColumn: "2", background: "var(--color-surface)", border: "1px solid var(--color-hairline)", borderRadius: "var(--radius-md)", padding: "var(--space-lg)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "baseline", marginBottom: "var(--space-md)" }}>
              <div><h2 className="text-subheading" style={{ margin: 0 }}>Invitation history</h2><p className="text-caption" style={{ color: "var(--color-ink-muted)", margin: "4px 0 0" }}>Select an organization to view its invitations.</p></div>
              <span className="text-caption" style={{ color: "var(--color-ink-muted)" }}>{invitations.length} total</span>
            </div>
            {!organizationId ? <p className="text-body-sm" style={{ color: "var(--color-ink-muted)" }}>No organization selected.</p> : invitationsLoading ? <p className="text-body-sm">Loading invitations…</p> : invitations.length === 0 ? <p className="text-body-sm" style={{ color: "var(--color-ink-muted)" }}>No invitations for this organization yet.</p> : <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>{invitations.map((invitation) => { const status = invitationStatus(invitation); return <div key={invitation.id} style={{ display: "grid", gridTemplateColumns: "minmax(150px, 1fr) auto auto", gap: 12, alignItems: "center", padding: "10px 0", borderBottom: "1px solid var(--color-hairline)" }}><div><div className="text-body-sm" style={{ fontWeight: 600 }}>{invitation.email}</div><div className="text-caption" style={{ color: "var(--color-ink-muted)" }}>Created {formatDate(invitation.createdAt)} · Expires {formatDate(invitation.expiresAt)}</div></div><span className="text-caption" style={{ color: status.color, background: status.background, borderRadius: 999, padding: "3px 8px", fontWeight: 700 }}>{status.label}</span><span className="text-caption" style={{ color: "var(--color-ink-muted)" }}>{formatDate(invitation.acceptedAt)}</span></div>; })}</div>}
          </section>
        </div>
      </div>
    </main>
